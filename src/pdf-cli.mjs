...
if(args.length<2||args.length>3){console.error('Usage: pdf-cli.mjs input.pdf output.json [pages: 1-5,8]');process.exitCode=1;}
 ...
 if(pages&&!/^\d+(?:-\d+)?(?:,\d+(?:-\d+)?)*$/.test(pages))throw Error('Invalid page range.');
 ...
 const out={schema:'m365-relay-pdf-v1',parser:'liteparse',parserVersion:'2.14.4',ocrEnabled:false,
 ...
 console.log(JSON.stringify({output,pages:out.pages.length,totalPages:out.totalPages,ocrEnabled:false}));
