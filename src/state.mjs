...
export async function acquireProcessLock(home) {
  ...
  for(let attempt=0;attempt<2;attempt++) {
    try { await mkdir(path); }
      ...
      throw new BridgeError('already_running','M365Relayの起動ロックがあります。動作中ならその起動ウィンドウを使用してください。異常終了後は配布フォルダーのRecover.cmdを開き、復旧に成功したら起動ファイル（M365Relay.cmdまたはRun.cmd）を開き直してください。',409);
    }
