...
export function runtimeGuidance(messages){
 ...
 const integrityNote='For an exact file copy, never recreate the source text with create_file or an edit tool. Ask VS Code to run the bundled byte-preserving helper: & $env:M365_RELAY_NODE (Join-Path $env:M365_RELAY_APP \'src/cli.mjs\') copy-verify <source> <new-destination>. It uses fs.copyFile without decoding, reads both files back, and compares byte counts and SHA-256. Do not overwrite an existing destination or report success when the comparison is not true. If Get-FileHash is used as a fallback, require exit code 0 and non-empty source/destination hashes; an empty HASH_MATCH=True after a command-not-found error is a failure.';
