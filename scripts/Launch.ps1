...
[CmdletBinding()]
param([ValidateSet('run','setup','init','open','diagnose','serve','recover-lock','help')][string]$Action = 'help', [string]$Workspace)
...
try {
    ...
    $env:NODE_PATH = $null
    if ($Workspace) {
        & (Join-Path $root 'runtime\node.exe') (Join-Path $root 'src\cli.mjs') $Action $Workspace
    } else {
