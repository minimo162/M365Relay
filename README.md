...
**0.2.2 / 実M365とVS Codeを接続した通し動作は未検証のプロトタイプです。**

この版はツール定義の説明項目に対応し、入力欄の改行読み取りと入力後の待機を修正しています。
...
ダウンロードしたArtifactsのZIPの中に、配布用`M365Relay-0.2.2-win-x64-<commit>.zip`と`.sha256`があります。
...
## 0.2.2: 入力が BRID で止まる場合の修正

0.2.1以前は3,000文字ずつ入力し、入力欄の `innerText` と元の文章を比較していました。
ブラウザーが改行を段落として扱うと `innerText` に追加の改行が入り、最初の3,000文字が正しく入力されても停止します。
今回の画像の末尾 `BRID` は、最初の3,000文字の境界と一致しました。文字数上限が3,000という意味ではありません。

0.2.2は段落・空行・明示的な改行を区別して読み取り、入力後は最大3秒の間、読み取りだけを繰り返して安定を確認します。
送信直前も同じ読み取り方法で全文を照合します。空白削除による比較、パーセント一致、欠落したままの送信、再挿入は行いません。

旧版の `Start-Bridge.cmd` をCtrl+Cで止め、この配布物を別フォルダーへ展開し `Start-Bridge.cmd` を起動してください。
起動表示が **M365Relay 0.2.2** になったことを確認します。
設定・`token.txt`・専用Edgeプロファイルは変更しません。途中まで入力された旧タブは手動送信しないでください。

同じエラーが出た場合は、新しいエラーの文字数・差分位置・文字種だけを共有してください。
依頼本文・トークン・DOMの全体・HTTP要求の全文は不要です。

検証方法と残る限界は [入力修正の記録](docs/input-compatibility.md) を参照してください。
実M365との接続、Windows版Edge、組織側の入力上限の検証を済ませたという意味ではありません。
...
## unsupported_schema の修正（0.2.1）

VS Codeの標準ターミナルが使う`enumDescriptions`など、選択肢の説明・表示用項目を受け付けるようにしました。
`enum`、型、必須項目、余分な引数の検査は維持しています。別の未対応スキーマがある場合は、
ツール名（または`response_format`）と既知の項目名をエラーに表示します。定義や値の全文は出しません。

更新時は旧`Start-Bridge.cmd`のウィンドウをCtrl+Cで終了し、新しい配布ZIPを別フォルダーへ展開して、
新しい`Start-Bridge.cmd`を起動してください。`%LOCALAPPDATA%\M365Relay`の設定・接続キーは削除しません。
保存先を環境変数で変更している場合は同じ値を使います。ソースファイルだけの差し替えは
配布ハッシュの不一致になるため行わないでください。

原因・検証範囲の詳細は、同梱の`docs/schema-compatibility.md`を参照してください。
