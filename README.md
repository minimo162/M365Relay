...
引数なしで新しく接続を開始すると、専用の既定workspaceを開きます。既に接続が起動している場合は、その接続を開始したときのフォルダーへ戻ります。次回起動時は、VS Codeで最後に開いた専用workspaceを明示的な引数なしで再利用します。存在しないフォルダーは推測で作り直さず、状態と次の操作を表示して停止します。
