...
**公開版0.2.25。自動更新、同梱Python、新規専用環境の初回モデル準備に対応しています。固定の文書課題では追加助言なしの完走を確認しました。幅広い作業での安定性・高速化は検証を続けています。**
...
引数なしで新しく接続を開始すると、専用の既定workspaceを開きます。既に接続が起動している場合は、その接続を開始したときのフォルダーへ戻ります。VS Codeのメニューで最後に切り替えたフォルダーを起動ファイルが記憶する機能は、現在はありません。
