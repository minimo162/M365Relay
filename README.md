...
0.2.21候補では、会話履歴・大きいツール結果の全文も要求ごとのTXTへ分離し、現在の指示と必要箇所の抜粋を本文へ置きます。専用VS Codeの入力枠は256000トークン、会話TXTは最大4MiBです。M365入力欄の120000文字上限は維持します。モデル自体のネイティブ容量を増やす機能ではありません。

現在は未公開の検証候補です。長文と画像の実M365搬送は確認済みですが、実VS Codeではcompact無しでExcel作成・計算まで進んだ一方、引用符の保持と画像保存が未達です。[設計と検証範囲](docs/context-attachments-design.md)を参照してください。
