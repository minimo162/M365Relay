...
台帳は`requests.json`を現行記録、`requests.archive.jsonl`を追記型の保管記録として使います。現行10,000件に達したとき、完了・結果不明・未送信の古い記録だけを保管へ移し、HMAC指紋と状態を残したまま新しい要求の場所を作ります。送信中の記録しか整理できない場合は`ledger_full`で停止し、台帳削除や依頼文の言い換えを回避策にしません。保管ファイルが途中で壊れている場合も重複判定を失わないよう起動を拒否します。

`/health`の`m365_state`と`model_state`は、過去の成功を永久に保持するフラグではありません。要求が送信後にタイムアウト、切断、応答検証失敗になった場合は`result_unconfirmed`/`not_verified`へ戻し、同じ要求を自動再送しません。指定モデルの選択確認に失敗した場合は`not_verified`/`unavailable`へ戻します。選択側と監視側は`copilot_model_unavailable`を含む共通のエラー契約を使います。

専用VS Code拡張は、通常のVS Code設定やDBを読まず、起動時に渡された専用`workspace-state.json`へ最後に観測した最初のworkspaceを保存します。引数なし起動では、その保存値を実在確認してから再利用し、削除・破損時は既定フォルダーを推測して作りません。明示的な起動引数は保存値より優先します。
