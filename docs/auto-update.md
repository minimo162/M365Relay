...
0.2.24ではRun.cmdの通常起動時に希望ポートが使用中なら、OSが確保した別のループバックポートを使用します。他のプロセスを停止せず、設定ファイルの希望ポートも変更しません。専用VS Codeの接続先と署名対象の起動情報に実ポートを保存し、再起動操作ではそのポートの所有証明を確認して画面へ戻ります。固定ポートを明示的に使用するserveコマンドは従来どおり競合をエラーにします。
